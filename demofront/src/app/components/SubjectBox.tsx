...
const Subject: React.FC<SubjectProps> = ({
  ...
  const handleFetchAndRedirect = (subjectId: string) => {
    // Redirect to the dynamic page with the subjectId as a path parameter
    router.push(`/demo_detail/dynamic/${subjectId}`);
  };
      ...
      {BoxSubject.map((box, index) => (
        <div key={index} className="rounded border shadow-md p-6">
          <div className="flex space-x-2 flex-wrap">
            ...
            <button 
            onClick={() => handleFetchAndRedirect(box.subject_id)}
