...
const SelectPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const storedSelectedSubjects = localStorage.getItem('selectedSubjectsData');
    if (storedSelectedSubjects) {
      setSelectSubjects(JSON.parse(storedSelectedSubjects));
      console.log(storedSelectedSubjects)
    }
  }, []);
  ...
  const fetchSubjectsFromDatabase = async () => {
    try {
      ...
      //console.log(subjectsWithSections);
    } catch (error) {
  ...
  const toggleSubjectSelection = (index: number) => {
    setSelectSubjects((prevSelectSubjects) => {
      let updatedSelectSubjects;
      if (prevSelectSubjects.includes(index)) {
        updatedSelectSubjects = prevSelectSubjects.filter((i) => i !== index);
      } else {
        updatedSelectSubjects = [...prevSelectSubjects, index];
      }
      // Store in localStorage
      localStorage.setItem('selectedSubjectsData', JSON.stringify(updatedSelectSubjects));
      return updatedSelectSubjects;
    });
  };
